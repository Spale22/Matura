...
namespace Matura_Zadatak_A28
{
    public partial class Kupovina : Form
    {
        ...
        {
            if (!string.IsNullOrEmpty(GV.Rows[e.RowIndex].Cells[5].Value.ToString()))
            {
                try
                {
                    conn.Open();
                    SqlCommand comm = new SqlCommand(@"SELECT jedinica_na_stanju FROM Proizvod WHERE proizvodID=@id", conn);
                    comm.Parameters.AddWithValue("@id", GV.Rows[e.RowIndex].Cells[0].Value);
                    int kolicina = Convert.ToInt32(comm.ExecuteScalar());
                    if (kolicina > Convert.ToInt32(GV.Rows[e.RowIndex].Cells[5].Value))
                        GV.Rows[e.RowIndex].Cells[5].Style.BackColor = Color.Green;
                    else
                        GV.Rows[e.RowIndex].Cells[5].Style.BackColor = Color.Red;
                }
